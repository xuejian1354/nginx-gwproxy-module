...
void ngx_gwproxy_write_handler(ngx_event_t *ev);

void ngx_stream_gw_proxy_handler(ngx_stream_session_t *s)
{
    ngx_str_t                      text = ngx_string("GW Proxy");
    ngx_buf_t                     *b;
    ngx_connection_t              *c;
    ...
    c->write->handler = ngx_gwproxy_write_handler;

    ngx_gwproxy_write_handler(c->write);
}
